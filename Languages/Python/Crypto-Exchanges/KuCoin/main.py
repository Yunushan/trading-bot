...
def _install_cbt_startup_window_suppression() -> None:
    ...
    debug_log_path = Path(os.getenv("TEMP") or ".").resolve() / "kucoin_window_events.log"
...
APP_USER_MODEL_ID = "KuCoin.TradingBot"
...
def _schedule_icon_enforcer(app: QApplication, window) -> None:  # noqa: ANN001
    ...
    def _attempt() -> None:
        ...
        if _env_flag("BOT_ENABLE_NATIVE_ICON"):
            _set_native_window_icon(window)
        if _env_flag("BOT_ENABLE_DELAYED_QT_ICON"):
            _apply_qt_icon(app, window)
        if state["remaining"] > 0:
...
def main() -> int:
    ...
    app.setApplicationName("KuCoin Trading Bot")
    app.setApplicationDisplayName("KuCoin Trading Bot")
    ...
    try:
        ...
        pass
    if not disable_app_icon and not icon.isNull():
        try:
    ...
    if sys.platform == "win32" and not disable_taskbar:
        ...
        def _apply_taskbar(attempts: int = 12) -> None:
            ...
            success = apply_taskbar_metadata(
                ...
                display_name="KuCoin Trading Bot",
                icon_path=icon_path,
