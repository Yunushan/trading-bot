    ...
    def launch_selected_bot(self) -> None:
        ...
        try:
            ...
            if self.selected_language == "python":
                ...
                if self.selected_exchange == "okx":
                    okx_icon_ico = REPO_ROOT / "assets" / "exchange_okx.ico"
                    okx_icon = REPO_ROOT / "assets" / "exchange_okx.png"
                    okx_icon_fallback_ico = BASE_DIR / "app" / "assets" / "okx_icon.ico"
                    okx_icon_fallback = BASE_DIR / "app" / "assets" / "okx_icon.png"
                    for candidate in (
                        okx_icon_fallback_ico,
                        okx_icon_ico,
                        okx_icon_fallback,
                        okx_icon,
                    ):
                        if candidate.is_file():
                            env.setdefault("OKX_BOT_ICON", str(candidate))
                            break
                if sys.platform == "win32":
