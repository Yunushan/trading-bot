...
_COMMON_FALLBACKS = ("mexc_icon.svg",)
...
def _candidate_directories() -> list[Path]:
    ...
    env_assets = os.getenv("MEXC_BOT_ASSETS")
    if env_assets:
...
def load_app_icon() -> QtGui.QIcon:
    """Load the MEXC app icon with platform-aware fallbacks."""
    env_icon_path = os.getenv("MEXC_BOT_ICON")
    if env_icon_path:
    ...
    if icon and not icon.isNull():
        return icon
    for env_var in ("APPDIR", "MEXC_BOT_APPDIR"):
        appdir = os.getenv(env_var)
        if appdir:
            icon_path = Path(appdir) / "mexc_icon.png"
    ...
    if fallback and not fallback.isNull():
        return fallback
    theme_icon = QtGui.QIcon.fromTheme("mexc")
