    ...
    *,
    ...
    try:
        ...
        _pause_for_close_uncertainty(
            self,
            f"{sym_norm} futures mode lookup failed before flat-leg purge: {exc}",
    ...
    try:
        ...
        _pause_for_close_uncertainty(
            self,
            f"{sym_norm} flat-leg purge grace is invalid: {exc}",
    ...
    try:
        ...
        _pause_for_close_uncertainty(
            self,
            f"{sym_norm} flat-leg purge threshold is invalid: {exc}",
            reconciliation_required=True,
        )
        return
    for leg_key, leg in list(self._leg_ledger.items()):
        ...
        try:
            ...
            _pause_for_close_uncertainty(
                self,
                f"{sym_norm}@{leg_interval} recorded leg quantity is invalid: {exc}",
                reconciliation_required=True,
            )
            continue
        ...
        if live_qty is None:
            _pause_for_close_uncertainty(
                self,
                f"{sym_norm}@{leg_interval} exchange quantity is unavailable during flat-leg purge",
                reconciliation_required=False,
            )
            continue
        ...
        try:
            ...
            _pause_for_close_uncertainty(
                self,
                f"{sym_norm}@{leg_interval} ledger entries are unavailable during flat-leg purge: {exc}",
                reconciliation_required=True,
            )
            continue
        if purge_grace_seconds > 0.0:
            ...
            for entry in entries or []:
                try:
                    ...
                    _pause_for_close_uncertainty(
                        self,
                        f"{sym_norm}@{leg_interval} ledger timestamp is invalid: {exc}",
                        reconciliation_required=True,
                    )
                ...
                if not math.isfinite(ts_val) or ts_val < 0.0:
                    _pause_for_close_uncertainty(
                        self,
                        f"{sym_norm}@{leg_interval} ledger timestamp is not finite and nonnegative",
                        reconciliation_required=True,
                    )
                    timestamp_invalid = True
                    break
            ...
            if newest_ts <= 0.0:
                try:
                    ...
                    _pause_for_close_uncertainty(
                        self,
                        f"{sym_norm}@{leg_interval} leg timestamp is invalid: {exc}",
                        reconciliation_required=True,
                    )
                    continue
                if not math.isfinite(newest_ts) or newest_ts < 0.0:
                    _pause_for_close_uncertainty(
                        self,
                        f"{sym_norm}@{leg_interval} leg timestamp is not finite and nonnegative",
                        reconciliation_required=True,
                    )
                    continue
        ...
        for entry in entries or []:
            try:
                ...
                _pause_for_close_uncertainty(
                    self,
                    f"{sym_norm}@{leg_interval} re-entry state cleanup failed: {exc}",
                    reconciliation_required=True,
                )
            try:
                ...
                _pause_for_close_uncertainty(
                    self,
                    f"{sym_norm}@{leg_interval} indicator close-state cleanup failed: {exc}",
                    reconciliation_required=True,
                )
            try:
                ...
                _pause_for_close_uncertainty(
                    self,
                    f"{sym_norm}@{leg_interval} flip-on-close queue update failed: {exc}",
                    reconciliation_required=True,
                )
