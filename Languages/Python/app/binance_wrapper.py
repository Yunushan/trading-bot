...
class BinanceWrapper:
    ...
    def close_futures_leg_exact(self, symbol: str, qty: float, side: str, position_side: str | None = None):
        ...
        try:
                    ...
                    if preferred_ps in ("LONG", "SHORT"):
                        ...
                        if side_up == 'SELL':
                            ...
                            if row_ps == "SHORT":
                                ...
                                include = True
                        if not include:
                            # Fallback for connectors that don't populate positionSide reliably.
            ...
            if live_qty > qty_tol:
                q = min(q, live_qty)
            # Do not early-skip when snapshots are missing or stale; still attempt a close order.
            if q <= qty_tol:
        ...
        try:
            ...
            for p in positions:
                try:
                    ...
                    if abs(amt) <= 0.0:
                        continue
                    raw_ps = str(p.get('positionSide') or p.get('positionside') or '').upper().strip()
...
def _bw_close_futures_position(self, symbol: str):
    ...
    for pos in infos or []:
        ...
        if abs(amt) <= 0:
            continue
        side, target_ps = _resolve_close(amt, pos.get('positionSide') or pos.get('positionside'))
        if side not in ('BUY', 'SELL'):
            continue
        qty = abs(amt)
