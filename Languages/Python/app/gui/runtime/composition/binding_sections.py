...
def _bind_window_and_chart_core(main_window_cls, g, modules: SimpleNamespace) -> None:
    ...
    window.runtime.bind_main_window_runtime(
        main_window_cls,
    ...
    account.account.bind_main_window_account_runtime(
        main_window_cls,
    ...
    chart.view.bind_main_window_chart_view_runtime(
        main_window_cls,
        chart_interval_options=g["CHART_INTERVAL_OPTIONS"],
        chart_market_options=g["CHART_MARKET_OPTIONS"],
    )
    chart.host.bind_main_window_chart_host_runtime(main_window_cls)
    chart.tab.bind_main_window_chart_tab(
        main_window_cls,
        chart_market_options=g["CHART_MARKET_OPTIONS"],
        chart_interval_options=g["CHART_INTERVAL_OPTIONS"],
    ...
    ui.tab.bind_main_window_tab_runtime(
        main_window_cls,
...
def _bind_dashboard_runtime(main_window_cls, g, modules: SimpleNamespace) -> None:
    ...
    dashboard.log.bind_main_window_dashboard_log_runtime(main_window_cls)
    dashboard.markets.bind_main_window_dashboard_markets_runtime(
        main_window_cls,
    ...
    dashboard.state.bind_main_window_dashboard_state_runtime(
        main_window_cls,
        load_position_allocations=g["_load_position_allocations"],
    )
    dashboard.strategy.bind_main_window_dashboard_strategy_runtime(
        main_window_cls,
        side_labels=g["SIDE_LABELS"],
    ...
    dashboard.indicator.bind_main_window_dashboard_indicator_runtime(
        main_window_cls,
        indicator_display_names=g["INDICATOR_DISPLAY_NAMES"],
        param_dialog_cls=g["ParamDialog"],
    )
    dashboard.actions.bind_main_window_dashboard_actions_runtime(main_window_cls)
    dashboard.chart.bind_main_window_dashboard_chart_runtime(
        main_window_cls,
        qt_charts_available=g["QT_CHARTS_AVAILABLE"],
    )
    dashboard.header.bind_main_window_dashboard_header_runtime(
        main_window_cls,
...
def _bind_bootstrap_backtest_and_chart_runtime(main_window_cls, g, modules: SimpleNamespace) -> None:
    ...
    window.bootstrap.bind_main_window_bootstrap_runtime(
        main_window_cls,
    ...
    window.init_finalize.bind_main_window_init_finalize_runtime(main_window_cls)
    ui.secondary_tabs.bind_main_window_secondary_tabs_runtime(main_window_cls)
    positions.tab.bind_main_window_positions_tab(
        main_window_cls,
        coerce_bool=g["coerce_bool"],
        pos_close_column=g["POS_CLOSE_COLUMN"],
        positions_worker_cls=g["_PositionsWorker"],
    )
    backtest.tab.bind_main_window_backtest_tab(
        main_window_cls,
    ...
    backtest.state.bind_main_window_backtest_state_runtime(
        main_window_cls,
        backtest_interval_order=g["BACKTEST_INTERVAL_ORDER"],
        side_labels=g["SIDE_LABELS"],
        symbol_fetch_top_n=g["_SYMBOL_FETCH_TOP_N"],
    )
    backtest.template.bind_main_window_backtest_template_runtime(
        main_window_cls,
        mdd_logic_options=g["MDD_LOGIC_OPTIONS"],
        mdd_logic_default=g["MDD_LOGIC_DEFAULT"],
        backtest_template_definitions=g["BACKTEST_TEMPLATE_DEFINITIONS"],
    ...
    backtest.execution.bind_main_window_backtest_execution_runtime(
        main_window_cls,
    ...
    chart.selection.bind_main_window_chart_selection_runtime(
        main_window_cls,
    ...
    chart.display.bind_main_window_chart_display_runtime(main_window_cls)
...
def _bind_runtime_control_and_service_helpers(main_window_cls, g, modules: SimpleNamespace) -> None:
    ...
    strategy.control.bind_main_window_control_runtime(
        main_window_cls,
        strategy_engine_cls=g["StrategyEngine"],
    ...
    account.balance.bind_main_window_balance_runtime(
        main_window_cls,
        normalize_connector_backend=g["_normalize_connector_backend"],
    )
    service.session.bind_main_window_session_runtime(
        main_window_cls,
        default_connector_backend=g["DEFAULT_CONNECTOR_BACKEND"],
        normalize_connector_backend=g["_normalize_connector_backend"],
        save_app_state_file=g["_save_app_state_file"],
    )
    strategy.ui.bind_main_window_strategy_ui_runtime(
        main_window_cls,
        account_mode_options=g["ACCOUNT_MODE_OPTIONS"],
        stop_loss_scope_options=g["STOP_LOSS_SCOPE_OPTIONS"],
    )
    service.api.bind_main_window_service_api_runtime(
        main_window_cls,
        save_app_state_file=g["_save_app_state_file"],
    )
    ui.ui_misc.bind_main_window_ui_misc_runtime(main_window_cls)
    strategy.override.bind_main_window_override_runtime(
        main_window_cls,
        format_indicator_list=g["_format_indicator_list"],
        normalize_connector_backend=g["_normalize_connector_backend"],
    ...
    service.status.bind_main_window_status_runtime(main_window_cls)
    strategy.stop_loss.bind_main_window_stop_loss_runtime(
        main_window_cls,
        normalize_stop_loss_dict=g["normalize_stop_loss_dict"],
        stop_loss_mode_order=g["STOP_LOSS_MODE_ORDER"],
        stop_loss_scope_options=g["STOP_LOSS_SCOPE_OPTIONS"],
    )
    strategy.controls.bind_main_window_strategy_controls_runtime(
        main_window_cls,
        side_labels=g["SIDE_LABELS"],
        normalize_stop_loss_dict=g["normalize_stop_loss_dict"],
        normalize_connector_backend=g["_normalize_connector_backend"],
    )
    strategy.indicator.bind_main_window_indicator_runtime(
    ...
    strategy.context.bind_main_window_strategy_context_runtime(
        main_window_cls,
...
def _bind_trade_positions_and_tail_runtime(main_window_cls, g, modules: SimpleNamespace) -> None:
    ...
    trade.runtime.bind_main_window_trade_runtime(
        main_window_cls,
        resolve_trigger_indicators=g["_resolve_trigger_indicators"],
        save_position_allocations=g["_save_position_allocations"],
        normalize_trigger_actions_map=strategy.indicator._normalize_trigger_actions_map,
        max_closed_history=g["MAX_CLOSED_HISTORY"],
    )
    positions.runtime.bind_main_window_positions(
        main_window_cls,
        resolve_trigger_indicators=g["_resolve_trigger_indicators"],
        max_closed_history=g["MAX_CLOSED_HISTORY"],
        stop_strategy_sync=strategy.control._stop_strategy_sync,
        pos_status_column=g["POS_STATUS_COLUMN"],
        save_position_allocations=g["_save_position_allocations"],
        normalize_indicator_values=g["_normalize_indicator_values"],
        derive_margin_snapshot=account.margin._derive_margin_snapshot,
        coerce_bool=g["coerce_bool"],
        format_indicator_list=g["_format_indicator_list"],
        collect_record_indicator_keys=strategy.indicator._collect_record_indicator_keys,
        collect_indicator_value_strings=strategy.indicator._collect_indicator_value_strings,
        collect_current_indicator_live_strings=strategy.indicator._collect_current_indicator_live_strings,
        dedupe_indicator_entries_normalized=strategy.indicator._dedupe_indicator_entries_normalized,
        numeric_item_cls=g["_NumericItem"],
    ...
    backtest.results.bind_main_window_backtest_results_runtime(
        main_window_cls,
        mdd_logic_labels=g["MDD_LOGIC_LABELS"],
        normalize_loop_override=main_window_cls._normalize_loop_override,
    )
    backtest.bridge.bind_main_window_backtest_bridge_runtime(
        main_window_cls,
        dbg_backtest_dashboard=g["_DBG_BACKTEST_DASHBOARD"],
        normalize_indicator_values=g["_normalize_indicator_values"],
        normalize_stop_loss_dict=g["normalize_stop_loss_dict"],
    )
    shared.config.bind_main_window_config(
        main_window_cls,
        normalize_stop_loss_dict=g["normalize_stop_loss_dict"],
        qt_charts_available=g["QT_CHARTS_AVAILABLE"],
        tradingview_supported=g["_tradingview_supported"],
    ...
    code.runtime.bind_main_window_code_runtime(main_window_cls)
    code.tab.bind_main_window_code(
        main_window_cls,
        ...
        launch_cpp_from_code_tab=code.runtime._launch_cpp_from_code_tab,
        launch_rust_from_code_tab=code.runtime._launch_rust_from_code_tab,
        refresh_code_language_card_release_labels=code.runtime._refresh_code_language_card_release_labels,
        refresh_dependency_usage_labels=g["_refresh_dependency_usage_labels"],
        base_project_path=g["_BASE_PROJECT_PATH"],
    )
    ui.theme.bind_main_window_theme_runtime(main_window_cls)
    desktop.service_bridge.bind_main_window_desktop_service_bridge(
        main_window_cls,
        desktop_service_client_factory=desktop.create_service_client,
    )
