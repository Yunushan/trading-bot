...
def _candidate_directories() -> list[Path]:
    ...
    gui_dir = here.parent.parent
    app_dir = gui_dir.parent
...
def _icon_from_data(data: bytes) -> QtGui.QIcon:
    if not data:
        return QtGui.QIcon()
...
def _icon_from_path(path: Path) -> QtGui.QIcon:
    ...
    try:
        ...
        pass
    try:
        return _icon_from_data(path.read_bytes())
    except Exception:
        return QtGui.QIcon()
