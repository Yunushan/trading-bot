...
- The official `MetaTrader5` Python package currently ships Windows x64 wheels. MT5 broker execution therefore requires a local Windows x64 MT5 terminal (or the Python service hosted on such a machine); macOS/Linux/mobile clients use the service API and do not execute MT5 locally. The connector bounds terminal inputs, symbols, order comments, and native-client timeouts before terminal calls.
