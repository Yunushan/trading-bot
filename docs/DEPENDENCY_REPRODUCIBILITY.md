...
`tools/verify_all.py` also runs the worktree summary and a blocking
workspace-hygiene check so dependency drift, missing client lockfiles,
generated artifacts, and missing local tooling are visible in one report. The
overall report does not pass while noisy generated artifacts are present.
