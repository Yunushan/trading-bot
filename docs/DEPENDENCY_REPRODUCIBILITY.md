...
matrix installs and tests it, the release-smoke job runs the pre-release
checks against it, and the Windows package-smoke job builds and launches the
PyInstaller executable with Python 3.15. Keep `.python-version`
...
The mobile production audit has no open exception for the current Expo/Metro build
graph. `apps/mobile-client/package.json` pins `nanoid` to 3.3.18, which is above
its published security floor, and overrides Metro's vulnerable `image-size` alias
with the reviewed `image-size-next@1.2.2` drop-in. The lockfile pins the package
tarball and integrity (`sha512-Pd3CJ2+Ifk2H2jWikkoz2BSZgnuF3Qsea4gQmj2gtiOtYpGWBl7elj8EXnFMiY5PaYNruTTLD0hQ0UWK7pz9xA==`)
at Metro's nested resolution path. This community-maintained fork carries fixes
for the ICNS/JXL/HEIF infinite-loop advisories (`GHSA-w3rx-r6r6-pgpr` and
`GHSA-5p2g-fcmc-qvqq`) while preserving Metro's `image-size@^1.x` API contract.
`tools/node-audit-policy.json` therefore contains an explicit empty exception set;
any new high/critical finding fails CI. The policy checker remains fail-closed for
unknown or malformed severity records and any future policy exception must still
be exact, time-bounded, and backed by executable mitigation evidence.

`tools/check_mobile_image_size_exposure.cjs` supplies executable supply-chain
evidence. It verifies that application source never imports `image-size`, that
Metro is the only lockfile consumer, that Metro resolves the exact patched fork,
that Metro has one guarded import in `Assets.js`, and that Metro rejects ICNS, JXL,
HEIF, HEIC, and AVIF before calling the parser. Re-review the pinned fork when a
new release or maintained upstream replacement becomes available, or if the
verified dependency boundary changes.
...
with `tools/check_node_audit_policy.cjs`; the policy evaluation remains
authoritative even when npm's advisory database changes.
