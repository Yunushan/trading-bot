...
| PRD-008 | P1 / IN_PROGRESS | Runtime + operator | L | 001 scope | Bounded same-user/host Live Spot owner gate; account-wide fencing and recovery contract open |
...
- Current increment: the workflow selects a versioned `observer-smoke` profile for the standalone read-only topology. It verifies service health and explicitly reports trading observations unavailable; its evidence is not promotion eligible. The strict quick/sustained trading-freshness requirements and genuine deployed observation remain open.
...
- Current increment: Live Spot obtains the signed Binance Spot UID, places the intent ledger under one UID-based path in the current OS user's home, and retains one wrapper-bound local owner lock through its execution session. An `active` or `recovery_required` marker blocks reacquisition until an offline operator attestation with a reconciliation reference; account-setting changes revoke the owner. This is a same-user/single-host gate, not exchange-side fencing. Legacy history migration, key rotation, verified exchange reconciliation, restored-state anti-rollback, other users/hosts/executors and the operator's approved scope remain open. Futures and Spot testnet retain their prior behavior.
...
- Current increment: `next_bar_open` validates finite, nonnegative fee/slippage inputs, rejects a nonpositive adverse fill price and fee arithmetic overflow, and has long/short cost-sensitivity regressions. The resulting fees are recorded with execution metadata. These are software guards, not calibrated market costs; indicator causality, gaps, market-specific frictions, provenance, holdout and live replay remain open.
...
- The offline `observer-smoke` contract covers service health only and cannot satisfy this task's sustained trading observations, deployment, rollback or capacity acceptance evidence.
...
### 2026-09-23 — follow-up ownership, observer and backtest checkpoint

- Starting source: `af720dfcc646b77d07cbd6d9c638dd0d07b2324c` on `codex/production-readiness-20260923`; the clean code commit is `3dc13908b39f6af759cd7c431e2be095366683d7`. No release artifact was produced. The prior clean-commit gate and 65/100 dated review above remain historical evidence for their tested revision.
- **PRD-008 IN_PROGRESS:** the Live Spot Python wrapper verifies a signed account UID, selects a fixed UID-based ledger under the current OS user's home, and retains an OS owner lock bound to one wrapper across activation and submission until release. A second wrapper or process under that user is blocked; owner release and crash both require offline operator reconciliation attestation before rearm. GUI stop/close keeps the active Spot owner, and account-setting changes revoke it. Synthetic offline tests cover distinct audit paths, second wrappers/processes, owner loss, changed credentials, recovery marker, Demo preservation and Futures close behavior. This does not fence another OS user, host, native runtime, external bot or valid key. Existing Spot history migration, credential rotation, machine-verified exchange reconciliation, restored-state anti-rollback and named operator/account/host scope remain open. No live Binance call or order occurred.
- **F14 observation and PRD-022 remain open:** the read-only deployment workflow now selects versioned `observer-smoke` service-health checks with `promotion_eligible=false` and explicitly unavailable trading observations. It cannot replace the quick/sustained trading-freshness gates, 30-day operational evidence or actual deployed topology and rollback proof. The earlier unseeded **1 of 4** trading-freshness finding remains relevant; no synthetic trading timestamps were added.
- **PRD-020 IN_PROGRESS:** the `next_bar_open` path now validates finite/nonnegative fee and slippage inputs, rejects adverse nonpositive fills and fee overflow, and records fee metadata. Synthetic long/short regressions verify adverse cost sensitivity. This does not establish empirical fees, spread, funding, latency or liquidity, nor close indicator causality, dataset provenance, holdout/walk-forward or live replay.
- Focused offline checks completed on this candidate tree: ownership/order/GUI/stop/provisioning/smoke tests **117 passed, 1 Windows symlink privilege skip, 220 subtests** before the final GUI invalidation regression; the final GUI suite **6 passed, 6 subtests**. The declared Python 3.14 account-observation suite **15 passed, 125 subtests**; backtest behavior **49 passed, 9 subtests**. The F14 lane reported **76 passed, 184 subtests**, with actionlint/ShellCheck. Changed Python Ruff checks and `git diff --check` passed.
- The staged-candidate complete source gate `.\.venv\Scripts\python.exe tools/verify_all.py --skip-promotion-evidence --json` returned exit 0 and `ok=true`: **41 checks**, **1,923 Python tests passed, 2 Windows symlink skips, 54.23% coverage**. Rust core, native C++, Tauri, service/web/mobile, lint/type and other required source checks passed. Only the nonblocking workspace-hygiene advisory reported ignored build/cache files; the external Rust evidence-import check was explicitly skipped. Two earlier combined runs exposed a Rust test-fixture failure; it was repaired and the stated result is the full rerun. No local report hash is claimed as durable production evidence.
- The strict clean-commit check `tools/check_operational_readiness.py --require-evidence --require-current-commit --require-clean-source --json` exited 1 for `3dc13908b39f6af759cd7c431e2be095366683d7`: `schema_ok=true`, `current_source_tree_clean=true`, `promotion_ready=false`. All four required artifacts were missing: `service-api-sustained-runtime`, `production-service-slo-window`, `service-config-backup-restore` and `incident-audit-continuity`. Policy SHA-256: `89fd34b17ac695febf98fd9e72ed21aa606cd5fbc7345244d85ea1e4ca47b02c`. The follow-up review was added after this clean-source check.
- A fresh public GitHub REST recheck still observed `main protected=false`, no repository rulesets, `production protection_rules=[]` and `can_admins_bypass=true`. No repository setting changed; administrator-only enforcement and named reviewer/release decisions remain open under PRD-001.
- **PRD-009 TODO — next independent offline candidate:** a read-only audit found that the cumulative Futures risk-stop path can silently skip a malformed position row and understate totals. Validate the whole snapshot and pause new entries on invalid data while preserving proven reducing closes. No PRD-009 implementation is claimed; operator risk values, durable kill state and account-wide scope remain open.
- Score change: the [2026-09-23 follow-up evidence review](PRODUCTION_READINESS_REVIEW_2026-09-23_FOLLOWUP.md) assigns **66/100 for unattended real-money production, NO-GO** at clean code commit `3dc13908b39f6af759cd7c431e2be095366683d7`. The prior dated **65/100 NO-GO** and 2026-09-17 **58/100 NO-GO** remain historical. No offline control, observer smoke or green test permits a live rollout.
