...
def _checks(root: Path, *, skip_slow: bool) -> list[Check]:
    ...
    checks = [
        ...
        Check("client dependency locks", (python, "tools/check_client_dependency_locks.py", "--json", "--strict"), root, required=False),
        Check("risky pattern audit", (python, "tools/audit_risky_patterns.py", "--json", "--summary"), root, required=False),
        Check("python lint", (python, "-m", "ruff", "check", "--config", "Languages/Python/pyproject.toml", "Languages/Python"), root),
        Check(
            "python type check",
