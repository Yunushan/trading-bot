...
def _check_release_platform_real_tests(root: Path) -> dict[str, Any]:
    ...
    issues = _missing_fragments(
        ...
        (
            ...
            "npx --prefix apps/web-dashboard playwright install firefox",
            "export DEBIAN_FRONTEND=noninteractive",
            "export PLAYWRIGHT_DOWNLOAD_CONNECTION_TIMEOUT=120000",
            "timeout --foreground --kill-after=30s 12m",
            "Install Playwright Google Chrome",
            "npx --prefix apps/web-dashboard playwright install chrome",
            "Install Playwright Microsoft Edge",
            "npx --prefix apps/web-dashboard playwright install msedge",
            "sudo -n apt-get -o Acquire::Retries=3 -o DPkg::Lock::Timeout=120 update",
            "sudo -n apt-get -o Acquire::Retries=3 -o DPkg::Lock::Timeout=120 install -y",
            *NATIVE_PARITY_DEPENDENCY_FRAGMENTS,
            "Install Qt for Windows native release smoke",
            "Install Qt for Unix native release smoke",
            "aqtinstall==3.3.0",
            "Qt6Config.cmake",
            "python tools/check_release_platform_matrix.py --schema-only",
            *SOURCE_SYNC_AUDIT_FRAGMENTS,
            "python tools/run_release_platform_probe.py",
            "--target-id \"${{ matrix.target.target_id }}\"",
            "--require-native-source-sync",
            "--output \"release-platform-evidence/${{ matrix.target.target_id }}.json\"",
            "Validate target evidence",
            "python tools/check_release_platform_matrix.py",
            "--require-evidence",
            "--require-current-commit",
            "--require-clean-source",
            "--target-filter \"${{ matrix.target.target_id }}\"",
    ...
    issues = _missing_fragments(
        ...
        (
            ...
            *NATIVE_PARITY_DEPENDENCY_FRAGMENTS,
            ...
            "Verify platform evidence run provenance",
            'gh run view "${PLATFORM_EVIDENCE_RUN_ID}"',
            "--json databaseId,status,conclusion,headSha,workflowName",
            "platform evidence run headSha must match the current release-evidence commit",
            "Release Platform Real Tests",
            *SOURCE_SYNC_AUDIT_FRAGMENTS,
            "gh run download",
            "release-platform-evidence",
    ...
    issues = _missing_fragments(
        ...
        (
            ...
            *NATIVE_PARITY_DEPENDENCY_FRAGMENTS,
            ...
            "Verify external evidence run provenance",
            'gh run view "${run_id}"',
            "--json databaseId,status,conclusion,headSha,workflowName",
            "Rust Native Live Smoke Evidence",
            "Rust Native Release Evidence",
            "run headSha must match the current promotion commit",
            *SOURCE_SYNC_AUDIT_FRAGMENTS,
